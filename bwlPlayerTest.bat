...
:OPTIMIZE
Echo testing: %test%
CALL tuo "testing_%test%" "%_enemy%" gw ordered -t %_threads% -o="%~dp0data"\%test%.txt climb %_climb%>>temp.txt
	set _temp=
	for /F "delims=" %%t in ('findstr "Optimized" temp.txt') do set _temp=!_temp!%%t	
	set cdeck1=%_temp:*: =%
	set cdeck2=%cdeck1:: =, %
	echo %Date% and %Time%,%test%,No Effect,%cdeck2%>>"Brawl Results".txt
	del temp.txt
GOTO %NEXTDEFENSE%
