...
CALL tuo "testing_%test%" "%_enemy%" gw -t %_threads% yf "%_yfort%" ef "%_efort%" -e "%Background%" -o="%~dp0data"\%test%.txt climb %_climb%>>temp.txt
	set _temp=
	for /F "delims=" %%t in ('findstr "Optimized" temp.txt') do set _temp=!_temp!%%t	
	set cdeck1=%_temp:*: =%
	set cdeck2=%cdeck1:: =, %
	echo %Date% and %Time%,%test%,%Background%,%cyfort%,%_efort%,%cdeck2%>>"Guild War".txt
	del temp.txt
GOTO %NEXTDEFENSE%
...
CALL tuo "testing_%test%" "%_enemy%" gw -t %_threads% yf "%_yfort%" -e "%Background%" -o="%~dp0data"\%test%.txt climb %_climb%>>temp.txt
	set _temp=
	for /F "delims=" %%t in ('findstr "Optimized" temp.txt') do set _temp=!_temp!%%t
	set cdeck1=%_temp:*: =%
	set cdeck2=%cdeck1:: =, %
	echo %Date% and %Time%,%test%,%Background%,%cyfort%,Core Attack,%cdeck2%>>"Guild War".txt
	del temp.txt
GOTO %NEXTDEFENSE%
